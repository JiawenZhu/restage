...
/*
 * The manifest is regenerated from what is actually on disk, so a partial run —
 * a Veo quota limit halfway through, say — leaves the gallery telling the truth
 * about which templates can be hovered rather than promising motion that is not
 * there.
 */
const { readdirSync } = await import('node:fs');
const files = readdirSync(OUT);
const clips = files.filter((f) => f.endsWith('.mp4')).map((f) => f.slice(0, -4)).sort();
const stills = files.filter((f) => f.endsWith('.jpg')).map((f) => f.slice(0, -4)).sort();
writeFileSync(
  new URL('../lib/templatePreviews.ts', import.meta.url).pathname,
  `/* Generated by scripts/make-template-previews.mts — do not edit by hand.\n` +
    ...
    `export const TEMPLATES_WITH_CLIP = new Set<string>(${JSON.stringify(clips)});\n` +
    `export const TEMPLATES_WITH_STILL = new Set<string>(${JSON.stringify(stills)});\n`,
);
console.log(`${stamp()}  manifest: ${clips.length} clips, ${stills.length} stills`);
